...
async def add_user_token(user_id, tokens=15):
    """Adds tokens to a user (default: 15 on verification)."""
    user_data = await users_collection.find_one({"user_id": user_id})
    if user_data:
        new_tokens = user_data.get("tokens", 0) + tokens
        await users_collection.update_one({"user_id": user_id}, {"$set": {"tokens": new_tokens}})
    else:
        await users_collection.insert_one({"user_id": user_id, "tokens": tokens})

async def get_user_tokens(user_id):
    """Retrieves the number of tokens a user has."""
    user_data = await users_collection.find_one({"user_id": user_id})
    return user_data["tokens"] if user_data else 0

async def deduct_token(user_id):
    """Deducts 1 token per file sent. If tokens = 0, request verification."""
    user_data = await users_collection.find_one({"user_id": user_id})
    if user_data and user_data.get("tokens", 0) > 0:
        new_tokens = user_data["tokens"] - 1
        await users_collection.update_one({"user_id": user_id}, {"$set": {"tokens": new_tokens}})
        return True
    return False

async def admin_add_tokens(user_id, tokens):
    """Admin can manually add tokens for premium users."""
    user_data = await users_collection.find_one({"user_id": user_id})
    if user_data:
        new_tokens = user_data.get("tokens", 0) + tokens
        await users_collection.update_one({"user_id": user_id}, {"$set": {"tokens": new_tokens}})
